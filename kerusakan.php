    ...
    <section class="content">
      ...
      <div class="box box-primary">
            <div class="box-header">
              <h3 class="box-title"><b>Data Keseluruhan Kerusakan</b></h3>
      ...
      ?>
        <div class="row">
          <div class="col-md-3">
            <a class='btn btn-primary btn-sm' href='add-kerusakan.php'>Add Kerusakan</a>
          </div> 
      ...
      <?php
          ...
          <table class="table table-hover table-bordered" id="view_table" style="font-size:small;">
              <thead style="background:#ccc;">
      			      <th width="4%">No</th>
                  <th width="13%">Data Perangkat</th>
                  <!--<th width="13%">Serial Number</th>-->
                  <th width="4%">Status</th>
                  <th>Tanggal</th>
                  <!--<th title="Lokasi Repair">Lokasi Repair</th>-->
                  <th>Kerusakan</th>
                  <th>User</th>
                  <th>Kota</th>
              ...
              if($kode_kota=="ALL"){
                  ...
                  */ 
                   ...
                   /*
                    ...
                    $data  = $dbfunc->list_kerusakan_daerah($kode_kota);
                   } 
                   */
              }  
            ...
            foreach ($data as $key => $row) {
                 # code...
              if(!empty($row["kota"])){
              $get_kota = $dbfunc->get_kota($row["kota"]);
             ...
             echo "<td><a href='#myModal2' id='custId' tabindex='-1' role='dialog'  data-toggle='modal'  data-id='$row[no_repair]/s'>$row[data_perangkat]</a></td>";
             #echo "<td>$row[sn]</td>";
             echo "<td>$row[status]</td>";
             echo "<td>".$row["tanggal_kirim"]."</td>";
             #echo "<td>".$row["lokasi_repair"]."</td>";
            ...
            $level    = $get_user["level"];
          #if ($level==1){
              echo "<a class='btn btn-primary btn-xs' href='#myModal2' id='custId' tabindex='-1' role='dialog'  data-toggle='modal' title ='edit' data-id='$row[no_repair]/edit'><span class='glyphicon glyphicon-pencil'></span></a>  <a class='btn btn-warning btn-xs' href='#myModal' id='custId' tabindex='-1' role='dialog'  data-toggle='modal' title ='Edit Process Repair' data-id='$row[no_repair]/edit2'><span class='glyphicon glyphicon-edit'></span></a>  <a href='#' id='$row[no_repair]' class='btn btn-danger btn-xs hapus' title='delete'><span class='glyphicon glyphicon-trash'></span></a>";
      		#}
          /*
          else{
...
<div class="modal fade" id="myModal2" role="dialog">
    <div class="modal-dialog" role="document">
      <form action="update-kerusakan.php" method="post" enctype="multipart/form-data" class="form-horizontal" >
     ...
     $(function() {
      $('#myModal2').on("shown.bs.modal", function(event){
        ...
        if (name=='edit'){
          $.ajax({
              type : 'POST',
              url : 'fetch_kerusakan.php', //Here you will fetch records 
...
<script>
  $(document).on('click', " tr td .hapus", function() {
    ...
    //if (sa == true) {
    $.post('delete_kerusakan.php', {id:del_id},function(data){
    //parent.slideUp('slow', function() {
    //$(this).remove();
     //$("#view_table").html(del_id);
    $(location).attr('href', 'kerusakan.php');
    //})
     });
   //} 
});
